...
outputSendCommand=$(aws ssm send-command --instance-ids "$Instance" --document-name "AWS-RunShellScript" --comment "Run echo command" --parameters commands='sudo /usr/local/bin/supervisorctl stop all> ScriptExecLog.txt'  --region $AWS_REGION --output text --query "Command.CommandId")
